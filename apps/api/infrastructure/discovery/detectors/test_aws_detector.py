            ...
            "Clusters": [
                {
                    ...
                    "DBName": "analytics",
                    "Endpoint": {"Address": "analytics-warehouse.abc123.us-east-1.redshift.amazonaws.com"}
                }
            ]
        }
...
class TestSecurityServices:
    ...
    def test_discover_kms_keys(self, mock_boto_client, aws_detector):
        ...
        kms_mock.list_keys.return_value = {"Keys": [{"KeyId": "arn:aws:kms:us-east-1:123456789012:key/12345678-1234-1234-1234-123456789012"}]}
...
class TestRelationshipDetection:
    ...
    def test_detect_ec2_to_ebs_relationships(self, aws_detector):
        ...
        assert len(edges) >= 1
        ebs_edges = [e for e in edges if e.from_node_key == "ec2:i-12345678" and e.to_node_key == "ebs:vol-12345678"]
    ...
    def test_detect_lambda_to_vpc_relationships(self, aws_detector):
        ...
        vpc_edges = [e for e in edges if e.from_node_key == "lambda:processor" and e.to_node_key == "vpc:vpc-12345678"]
    ...
    def test_detect_cloudfront_to_s3_relationships(self, aws_detector):
        ...
        cf_s3_edges = [e for e in edges if e.from_node_key == "cloudfront:E123ABCD" and e.to_node_key == "s3:my-bucket"]
...
class TestFullDiscoveryWorkflow:
    ...
    async def test_full_discovery_workflow(self, mock_boto_client, aws_detector):
        ...
        # Mock Lambda
        mock_client.get_paginator.return_value.paginate.return_value = [{"Functions": []}]
